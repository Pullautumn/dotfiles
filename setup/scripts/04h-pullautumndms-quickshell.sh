...
AUR_HELPER="paru"
section "Pullautumn DMS" "Core Components"
...
if [[ "${PULLAUTUMN_DMS_GIT:-0}" == "1" ]]; then
    log "Environment PULLAUTUMN_DMS_GIT=1 detected. Using git packages..."
...
# --- Dotfiles & Wallpapers ---
section "Pullautumn DMS" "Dotfiles & Wallpapers"
...
section "Pullautumn DMS" "File Manager & Terminal"
...
TERM_PKGS="xdg-terminal-exec fuzzel wf-recorder wl-screenrec-git ttf-jetbrains-maple-mono-nf-xx-xx eza zoxide starship jq fish libnotify timg imv cava imagemagick wl-clipboard cliphist pullautumn-contrib-git"

echo "$TERM_PKGS" >> "$VERIFY_LIST"
exe as_user "$AUR_HELPER" -S --noconfirm --needed $TERM_PKGS

# pullautumn-contrib
as_user pullautumn link
...
as_user sed -i "s/pullautumn/$TARGET_USER/g" "$HOME_DIR/.config/gtk-3.0/bookmarks"

# --- Flatpak & Theme Integration ---
section "Pullautumn DMS" "Flatpak & Theme Integration"
...
force_copy "$PARENT_DIR/resources/必看-Pullautumn-DMS-Niri使用方法.txt" "$HOME_DIR"

# niri blur toggle 脚本
curl -L pullautumn.xyz/niri-blur-toggle | as_user bash
