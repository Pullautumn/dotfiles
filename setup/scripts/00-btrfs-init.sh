...
# 部署主撤销脚本
if [ -f "$UNDO_SRC" ]; then
    exe cp "$UNDO_SRC" "$BIN_DIR/pullautumn-undochange"
    exe chmod +x "$BIN_DIR/pullautumn-undochange"
    success "Installed 'pullautumn-undochange' command."
else
...
if [ -f "$DE_UNDO_SRC" ]; then
    exe cp "$DE_UNDO_SRC" "$BIN_DIR/pullautumn-de-undochange"
    exe chmod +x "$BIN_DIR/pullautumn-de-undochange"
