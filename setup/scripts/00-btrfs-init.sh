...
if snapper list-configs | grep -q "root "; then
    if ! snapper -c root list --columns description | grep -q "Before Pullautumn Setup"; then
        if exe snapper -c root create --description "Before Pullautumn Setup"; then
...
if snapper list-configs | grep -q "home "; then
    if ! snapper -c home list --columns description | grep -q "Before Pullautumn Setup"; then
        if exe snapper -c home create --description "Before Pullautumn Setup"; then
...
exe mkdir -p "$BIN_DIR"
if [ -f "$UNDO_SRC" ]; then exe cp -f "$UNDO_SRC" "$BIN_DIR/pullautumn-undochange" && exe chmod +x "$BIN_DIR/pullautumn-undochange"; fi
if [ -f "$DE_UNDO_SRC" ]; then exe cp -f "$DE_UNDO_SRC" "$BIN_DIR/pullautumn-de-undochange" && exe chmod +x "$BIN_DIR/pullautumn-de-undochange"; fi
