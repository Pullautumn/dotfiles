...
RESTORE_SCRIPT="$HOME/.config/scripts/matugen-restore.sh"
...
# 还原配色选项
if [ "$IS_CN" = true ]; then
    RESTORE_OPTION=">> 还原自定义配色"
else
    RESTORE_OPTION=">> Restore Custom Colors"
fi
...
# --- 4. Fuzzel 菜单 ---
SELECTED_LINE=$(echo "$OPTIONS" | fuzzel -d --prompt="$PROMPT_TEXT" --lines=15)
...
if [[ "$SELECTED_LINE" == *">>"* ]]; then
    if [[ "$SELECTED_LINE" == *"亮色"* ]] || [[ "$SELECTED_LINE" == *"Light"* ]]; then
        ...
        REAL_VALUE="dark"
    elif [[ "$SELECTED_LINE" == *"还原"* ]] || [[ "$SELECTED_LINE" == *"Restore"* ]]; then
        REAL_VALUE="restore"
...
if [ -n "$REAL_VALUE" ]; then
    ...
    if [[ "$REAL_VALUE" == "restore" ]]; then
        "$RESTORE_SCRIPT"
        exit 0

    elif [[ "$REAL_VALUE" == "regenerate" ]]; then
