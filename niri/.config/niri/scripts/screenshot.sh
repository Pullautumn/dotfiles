...
SHOTEDITOR_DEFAULT="markpix" # 默认截图编辑器：swappy 或 satty 或 markpix
...
# 长截图脚本路径（根据实际情况修改）
LONGSHOT_SCRIPT="${LONGSHOT_SCRIPT:-$HOME/.config/niri/scripts/longshot.sh}"
...
if [[ "$LOCALE" == zh* ]]; then
  ...
  LABEL_LONGSHOT="Long Screenshot" # 新增
...
choose_editor() {
  local choice
  choice="$(menu_prompt "$PROMPT_EDITOR" "swappy" "satty" "markpix" "$LABEL_BACK")"
  case "$choice" in
  swappy | Swappy)
    SHOTEDITOR="swappy"
...
edit_from_clipboard() {
  ...
  swappy)
    ...
    ;;
  markpix)
    wl-paste -t image/png | markpix -f - -o "$dst"
    ;;
  *)
    echo "Unknown SHOTEDITOR: $SHOTEDITOR (use satty or swappy)" >&2
    return 0
...
run_niri_flow() {
  ...
  while :; do
    ...
    "$LABEL_LONGSHOT")
      # 调用长截图脚本
      if [[ -x "$LONGSHOT_SCRIPT" ]]; then
        exec "$LONGSHOT_SCRIPT"
      else
        echo "长截图脚本不存在或无执行权限: $LONGSHOT_SCRIPT" >&2
        if command -v notify-send >/dev/null 2>&1; then
          notify-send "错误" "长截图脚本未找到"
        fi
      fi
      return 0
      ;;
...
grim_capture_and_maybe_edit() {
  ...
  if [[ "$need_edit" == "yes" ]]; then
    ...
    *)
    ...
    shot="$SCREEN_DIR/Screenshots_$ts.png"
